...
def prepare_tqa_dataset(dataset, tokenizer, format_func): 
    ...
    for qa_pair in dataset: 
        ...
        for answer in qa_pair['correct_answers']: 
            complete_qa = format_func(question, answer)
            complete_qa_tokens = tokenizer(complete_qa, return_tensors = 'pt').input_ids
            target_tokens.append(complete_qa_tokens)
        for answer in qa_pair['incorrect_answers']: 
            complete_qa = format_func(question, answer)
            complete_qa_tokens = tokenizer(complete_qa, return_tensors = 'pt').input_ids
            source_tokens.append(complete_qa_tokens)
...
def get_activations(model, tokens, use_cache=False, past_key_values=None): 
    ...
    with torch.no_grad():
        ...
        with TraceDict(model, ckpts, retain_output=False, retain_input=True) as ret:
            outputs = model(
                tokens,
                use_cache=use_cache,
